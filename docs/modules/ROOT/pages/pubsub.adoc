...
You need to use the module `quarkus-pubsub-push-vertx-http` as a dependency in your own code to use this functionality. Previously the push logic was automatically packaged as a dependency, however this was found to conflict with the usage of Pubsub from Google Cloud Functions. Include the following in your project pom.xml (or an equivalent for Gradle or other build tools):

```xml
        <dependency>
            <groupId>io.quarkiverse.googlecloudservices</groupId>
            <artifactId>quarkus-google-cloud-pubsub-push-vertx-http</artifactId>
        </dependency>
```
