...
Another solution, is to produce a `CredentialsProvider` with a priority > 0 - the highest priority
will be used and the default provider has priority 0.

For example, this uses the `quarkus.google.cloud.service-account-encoded-key` property to authenticate
using a Base64 encoded Google Cloud service account key.

[source,java]
----
@ConfigProperty(name = "quarkus.google.cloud.service-account-encoded-key")
String base64Key;
...
public CredentialsProvider credentialsProvider() throws IOException {
    final InputStream keyStream = new ByteArrayInputStream(Base64.getDecoder().decode(base64Key));
        final GoogleCredentials credentials = GoogleCredentials.fromStream(keyStream).createScoped(
