...
* Using `QuarkusPubSub` subscribers can be registered to handle incoming messages. The subscriber methods automatically
  use the pull of push versions dependening on the `...push.enabled` flag.
...
* Using pull-subscriptions for testing purposes. For the `%test` profile, set the `...push.enabed` flag to false. This
